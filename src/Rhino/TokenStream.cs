			...
			for (; ; )
			{
				...
				if (identifierStart)
				{
					...
					if (!containsEscape)
					{
						...
						if (result != Token.EOF)
						{
							if ((result == Token.LET || result == Token.YIELD) && parser.compilerEnv.LanguageVersion < LanguageVersion.VERSION_1_7)
							{
							...
							{
								if (!parser.compilerEnv.ReservedKeywordAsIdentifier)
								{
									return result;
								}
							}
...
retry_break: ;
		...
		{
			if (parser.compilerEnv.RecordingComments && sourceReader != null)
			{
