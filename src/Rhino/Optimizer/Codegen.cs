...
namespace Rhino.Optimizer
{
	...
	public class Codegen : Evaluator
	{
		...
		private Type GenerateCode(string encodedSource)
		{
			...
			if (hasFunctions)
				constructor = mainClass.DefineConstructor(MethodAttributes.Public | MethodAttributes.HideBySig, CallingConventions.Standard, new[] { typeof (Scriptable), typeof (Context), typeof (int) });

			var regExpInit = EmitRegExpInit(mainClass);
			foreach (var n in scriptOrFnNodes)
			{
				var parameterTypes = GetParameterTypes(n);
				var bodyMethodName = GetBodyMethodName(n);
				mainClass.DefineMethod(bodyMethodName, MethodAttributes.Private | MethodAttributes.Static, typeof(object), parameterTypes);

				if (n.GetType() == Token.FUNCTION)
				{
					var ofn = OptFunctionNode.Get(n);
					mainClass.DefineMethod(GetFunctionInitMethodName(n), MethodAttributes.Private | MethodAttributes.Final, typeof (void), new[] { typeof (Context), typeof (Scriptable) });
					if (ofn.IsTargetOfDirectCall())
						mainClass.DefineMethod(GetDirectCtorName(n), MethodAttributes.Public | MethodAttributes.Static, typeof (object), parameterTypes);
					if (ofn.fnode.IsGenerator())
						mainClass.DefineMethod(bodyMethodName + "_gen", MethodAttributes.Private | MethodAttributes.Static, typeof (object), new[] { mainClass, typeof (Context), typeof (Scriptable), typeof (object), typeof (object), typeof (int) });
				}
			}
			for (int i = 0; i < scriptOrFnNodes.Length; i++)
			{
				...
				var parameterTypes = GetParameterTypes(n);
				var bodyMethodName = GetBodyMethodName(n);
				var functionMethod = (MethodBuilder) mainClass.GetMethod(bodyMethodName, BindingFlags.NonPublic | BindingFlags.Static, null, parameterTypes, null);
				System.Diagnostics.Debug.Assert(functionMethod != null);
				if (n.GetType() == Token.FUNCTION)
				{
					var ofn = OptFunctionNode.Get(n);
					var functionInitMethod = (MethodBuilder) mainClass.GetMethod(GetFunctionInitMethodName(n), BindingFlags.NonPublic, null, new[] { typeof (Context), typeof (Scriptable) }, null);
					System.Diagnostics.Debug.Assert(functionInitMethod != null);
					GenerateFunctionInitBody(regExpInit, functionInitMethod, n);
					if (ofn.IsTargetOfDirectCall())
					{
						var directConstructor = (MethodBuilder) mainClass.GetMethod(GetDirectCtorName(n), BindingFlags.Public | BindingFlags.Static, null, parameterTypes, null);
						System.Diagnostics.Debug.Assert(directConstructor != null);
						EmitDirectConstructorBody(directConstructor, functionMethod, n);
					}
				}

				var isGenerator = IsGenerator(n);
				var bodygen = BodyCodegen.CreateBodyCodegen(this, n, i, constructor, regExpInit, mainClass, compilerEnv, isGenerator);
				try
				{
					var method = isGenerator
						// All functions in the generated bytecode have a unique name. 
						// Every generator has a unique prefix followed by _gen
						? (MethodBuilder) mainClass.GetMethod(bodyMethodName + "_gen", BindingFlags.NonPublic | BindingFlags.Static, null, new[] { mainClass, typeof (Context), typeof (Scriptable), typeof (object), typeof (object), typeof (int) }, null)
						: functionMethod;

					bodygen.GenerateMethodBody(method);

					if (isGenerator)
					{
						// generate the user visible method which when invoked will
						// return a generator object
						bodygen.Clone().GenerateGeneratorBody(functionMethod);
					}
				}
				catch (ClassFileWriter.ClassFileFormatException e)
				{
			...
			if (hasFunctions)
				GenerateFunctionConstructorBody(idField, constructor);
		...
		private void EmitDirectConstructorBody(MethodBuilder directConstructor, MethodInfo methodBody, ScriptNode functionNode)
		{
			var il = directConstructor.GetILGenerator();
			...
			var argCount = functionNode.GetParamCount();
			for (var i = 0; i < argCount; i++)
			{
				il.EmitLoadArgument(4 + i*2);
				il.EmitLoadArgument(5 + i*2);
			}
			il.EmitLoadArgument(4 + argCount*2);
			il.Emit(OpCodes.Call, methodBody);
		...
		private static bool IsGenerator(ScriptNode node)
		{
		...
		private MethodBuilder GenerateCallMethod(CachingTypeBuilder tb, FieldInfo ifField)
		{
			...
			method.DefineParameter(0, ParameterAttributes.None, "script");
			method.DefineParameter(1, ParameterAttributes.None, "ctx");
			method.DefineParameter(2, ParameterAttributes.None, "scope");
			method.DefineParameter(3, ParameterAttributes.None, "thisObj");
			method.DefineParameter(4, ParameterAttributes.None, "args");
...
//								// get array[p]
		...
		private void GenerateFunctionConstructorBody(FieldInfo idField, ConstructorBuilder constructor)
		{
			var il = constructor.GetILGenerator();
			...
			for (var i = start; i < end; i++)
			{
				...
				il.Emit(OpCodes.Call, mainClass.GetMethod(GetFunctionInitMethodName(ofn.fnode)));
				il.Emit(OpCodes.Ret);
			}
		}

		private void GenerateFunctionInitBody(MethodInfo regExpInit, MethodBuilder method, ScriptNode functionNode)
		{
			var il = method.GetILGenerator();
			...
			if (functionNode.GetRegExpCount() != 0)
			{
				il.Emit(OpCodes.Ldarg_1); //cx
...
*/
		...
		private string GetFunctionInitMethodName(ScriptNode n)
		{
